...
class SpikeInterfaceDataModel(AbstractDataModel):
    ...
    @override
    def get_time_from_sample(self, sample: int) -> float:
        """Time in seconds of a sample index, using SpikeInterface's clock.

        Honors a non-zero start time or an explicit time vector when present.
        """
        return float(self.first_recording.sample_index_to_time(sample, segment_index=0))

    @override
    def get_sample_from_time(self, time: float) -> int:
        """Sample index of a time in seconds, using SpikeInterface's clock.
